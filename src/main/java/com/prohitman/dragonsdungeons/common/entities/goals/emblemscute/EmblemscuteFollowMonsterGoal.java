...
public class EmblemscuteFollowMonsterGoal extends Goal {
    private final EmblemscuteEntity mob;
    private final Predicate<Mob> followPredicate;
    @Nullable
    private Monster followingMob;
     ...
     */
    public EmblemscuteFollowMonsterGoal(EmblemscuteEntity pMob, double pSpeedModifier, float pStopDistance, float pAreaSize) {
        this.mob = pMob;
        this.followPredicate = (p_25278_) -> {
            return p_25278_ != null && pMob.getClass() != p_25278_.getClass() && !pMob.getHiding();
        };
    ...
    public boolean canUse() {
        List<Monster> list = this.mob.level().getEntitiesOfClass(Monster.class, this.mob.getBoundingBox().inflate((double)this.areaSize), this.followPredicate);
        if (!list.isEmpty()) {
    ...
    public boolean canContinueToUse() {
        return this.canUse() && this.followingMob != null && !this.navigation.isDone() && this.mob.distanceToSqr(this.followingMob) > (double)(this.stopDistance * this.stopDistance);
    }
