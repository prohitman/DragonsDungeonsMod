...
public class FollowLeaderGoal extends Goal {
    private final WargEntity mob;
    private final Predicate<WargEntity> followPredicate;
    @Nullable
    private WargEntity followingMob;
     ...
     */
    public FollowLeaderGoal(WargEntity pMob, double pSpeedModifier, float pStopDistance, float pAreaSize) {
        this.mob = pMob;
        this.followPredicate = WargEntity.LEADER_WARG_PREDICATE;
    ...
    public boolean canUse() {
        if(this.mob.isLeader() || this.mob.isTame()){
            return false;
        }

        List<WargEntity> list = this.mob.level().getEntitiesOfClass(WargEntity.class, this.mob.getBoundingBox().inflate((double)this.areaSize), this.followPredicate);
        if (!list.isEmpty()) {
            for(WargEntity mob : list) {
                if (!mob.isInvisible() && mob != this.mob) {
    ...
    public boolean canContinueToUse() {
        return !this.mob.isLeader() && !this.mob.isTame() && this.followingMob != null /*&& !this.navigation.isDone()*/ && this.mob.distanceToSqr(this.followingMob) > (double)(this.stopDistance * this.stopDistance);
    }
