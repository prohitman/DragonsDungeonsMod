...
public class FlaskOfAcid extends Item {
    public FlaskOfAcid(Properties properties) {
        super(properties);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level worldIn, Player playerIn, InteractionHand handIn) {
        ...
        if (!worldIn.isClientSide) {
            FlaskOfAcidProjectile acidBombEntity = new FlaskOfAcidProjectile(worldIn, playerIn);
            acidBombEntity.setItem(itemstack);
            acidBombEntity.shootFromRotation(playerIn, playerIn.getXRot(), playerIn.getYRot(), -20.0F, 0.5F, 1.0F);
            worldIn.addFreshEntity(acidBombEntity);
        }
